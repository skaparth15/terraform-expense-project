resource "aws_ec2_instance_state" "frontend_instance" {
  instance_id = data.aws_ssm_parameter.frontend_instance_id.value
  state       = "stopped"
}


resource "aws_ami_from_instance" "frontend_ami" {
  name               = "expense-frontend-ami"
  source_instance_id = data.aws_ssm_parameter.frontend_instance_id.value
  depends_on = [ aws_ec2_instance_state.frontend_instance ]

}


resource "aws_lb_target_group" "frontend-expense_alb" {
...
resource "aws_launch_template" "frontend" {
  name = "${var.project_name}-frontendconf"
...
resource "aws_autoscaling_group" "frontend" {
  ...
  vpc_zone_identifier       = split(",", data.aws_ssm_parameter.subnet_frontend_ids.value)
...
resource "aws_lb_listener_rule" "host_based_weighted_routing" {
  listener_arn = data.aws_ssm_parameter.frontend_alb_listener.value
  priority     = 99

  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.frontend-expense_alb.arn
  }

  condition {
    host_header {
      values = ["web.ilovedevops.fun"]
    }
  }
}
